...
| **관리 화면(`ts_console.ps1`)** | 한 번도 실행하지 않음. 아래 코드 리뷰만 거쳤습니다 |

`ts_console.ps1` 은 **작성 환경에 PowerShell 이 없어 문법 검사조차 하지 못했습니다.**
괄호·따옴표 균형만 기계적으로 확인했고, 나머지는 아래 리뷰가 전부입니다. 처음 실행할 때는
`-Once` 를 붙여 한 화면만 뽑아 보는 편이 안전합니다.
