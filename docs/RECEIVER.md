...
**`.git` 을 지워도 스냅샷 크기 아래로는 내려가지 않습니다.** 작업 트리가 6.3 GB 이고,
초기화 직후 첫 커밋을 하면 `.git` 이 다시 그만큼 찹니다. mp3·mp4·png 는 git 이 압축도
델타도 못 하기 때문입니다.

```
초기화 직후 바닥값 ≈ 작업 트리 6.3 GB + .git 약 6 GB   ≈ 12 GB
세대 하나당 추가                                        ≈ 6.3 GB
```
...
32,000개 파일 · 6.3 GB 를 풀고 `git add -A` 하는 데 수 분이 걸립니다. 작업 정의의
`MultipleInstancesPolicy=IgnoreNew` 가 앞선 회차와 겹치는 것을 막습니다.
