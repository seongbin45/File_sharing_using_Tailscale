...
실측된 한 회차(6.30 GB 대상, 파일 32,095개). 보내는 쪽은 로그의 타임스탬프를 그대로
뺀 값입니다:

| 단계 | 소요 |
|---|---|
| 압축 (7z `-mx=5`) → 4.74 GB | **4분 02초** |
| Taildrop 전송 (실효 7.18 MB/s) | **11분 00초** |
| 보내는 쪽 합계 | **15분 03초** |
| 받는 쪽 해제 · 커밋 · 태그 (첫 회차) | 약 16분 |
| 받는 쪽 해제 · 커밋 · 태그 (이후) | **2분 55초** |

받는 쪽 처리는 보내는 쪽과 겹치지 않습니다. 압축이 도착한 뒤 다음 수신 회차에서 처리하므로,
한 사이클이 끝나는 데까지는 20분 안팎입니다.

**받는 쪽은 첫 회차만 무겁습니다.** 전량을 새로 객체화하기 때문이고, 이후에는 git 이 바뀐
것만 저장해 3분대로 끝납니다. 90일 초기화 직후 한 번만 다시 16분이 듭니다.
...
| [docs/VERIFICATION.md](docs/VERIFICATION.md) | 무엇이 실기에서 검증됐고 무엇이 아닌지, **고친 뒤 무엇을 돌릴지** |
...
즉시 실행은 6 GB 를 압축·전송하므로 15분 안팎 걸립니다(실측: 압축 4분 + 전송 11분).
...
없었습니다. 전체 스냅샷은 한 회차가 6 GB · 15분이라 그럴 수 없습니다.
