...
전송 시간은 압축 크기를 실측 속도(7.18 MB/s)로 나눈 값이 기준입니다. 원본 6.3 GB 를
4.74 GB 로 압축하는 데 4분, 전송에 11분, 합계 15분이었습니다. 감당이 안 되면
`ts_backup_task.xml` 의 `<DaysInterval>` 을 늘리거나 `EXCLUDE_LIST` 를 도입해야 합니다.
...
- 6 GB 규모면 한 회차가 15분 안팎이므로 `timeout /t 90` 뒤에는 아직 실행 중입니다.
